...
This folder contains the property-extraction workflow. Its local numbering already runs continuously from `01` to `14`.

The pipeline consumes shared preprocessing outputs together with synthesis-side sample context and writes its results back into the same paper folder under `property/<stage_name>/`.

## Required Folder Pattern

Each paper folder is expected to live under one mining root, for example:

- `data/mining/<paper_id>_<paper_name>/preprocess/cut_property/<paper_id>.md`
- `data/mining/<paper_id>_<paper_name>/preprocess/cut/<paper_id>_cut.md`

The property workflow also expects synthesis-side sample information in `Synthesis/letter_table/<id>.csv`.
...
If multiple stages need the same non-prompt helper, add it to one of these modules instead of duplicating it in a stage script.
...
```bash
python property_mining/run_property_pipeline.py --root data/mining
```

Run only a subrange:

```bash
python property_mining/run_property_pipeline.py --root data/mining --start-step 5 --end-step 14
```

## Dependencies

- Root dependency file: [`../requirements-property.txt`](../requirements-property.txt)
